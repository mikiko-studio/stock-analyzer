...
# ── Navigation Cards (クリックでページ遷移) ───────────────────────────────────
col1, col2 = st.columns(2)
with col1:
    with st.container(border=True):
        st.page_link("pages/1_🎯_dividend_screener.py", label="🎯 高配当スクリーナー")
        st.markdown("""
3層フィルターで選ぶ「鉄壁高配当株」
...
""")
    with st.container(border=True):
        st.page_link("pages/3_🏹_bottom_screener.py", label="🏹 シグナル・ハンター")
        st.markdown("""
...
with col2:
    with st.container(border=True):
        st.page_link("pages/2_📐_buffett_screener.py", label="📐 三角測量スクリーナー")
        st.markdown("""
...
""")
    with st.container(border=True):
        st.page_link("pages/4_⚖️_portfolio_dashboard.py", label="⚖️ ポートフォリオ診断")
        st.markdown("""
...
# ── Top 5 Results from each screener ─────────────────────────────────────────
st.subheader("📊 各スクリーナー Top5 銘柄")

tab_div, tab_buf, tab_bot = st.tabs([
    "🎯 高配当スクリーナー",
    "📐 三角測量スクリーナー",
    "🏹 シグナル・ハンター",
])
...
with tab_buf:
    ...
    if not buf_results:
        st.info("⏳ 未実行 — 三角測量スクリーナーを実行するとTop5がここに表示されます")
    else:
        top5 = buf_results[:5]
        cols = st.columns(min(len(top5), 5))
        for i, r in enumerate(top5):
...
with tab_bot:
    bot_results = st.session_state.get("bottom_results", [])
    if not bot_results:
        st.info("⏳ 未実行 — シグナル・ハンターを実行するとTop5がここに表示されます")
    else:
        signals = [r for r in bot_results if r.get("buy_signal")]
        if signals:
            st.success(f"🟢 {len(signals)} 件の買いシグナルを検出中")
        display = (signals if signals else bot_results)[:5]
        ...
        for i, r in enumerate(display):
            with cols[i]:
                ...
                ma_str = f"{ma_dev:.1f}%" if ma_dev is not None else "—"
                badge = "🟢" if r.get("buy_signal") else "⬜"
                with st.container(border=True):
                    st.markdown(f"**{r['symbol']}** {badge}")
                    st.caption(r.get("name", "")[:12])
                    st.metric("RSI(14)", rsi_str)
                    st.caption(f"MA乖離: {ma_str}")
...
if not frames:
    ...
    with dl_col2:
        ...
        with pd.ExcelWriter(xlsx_buf, engine="openpyxl") as writer:
            if df_div is not None:
                df_div.drop(columns=["スクリーナー"]).to_excel(writer, sheet_name="高配当スクリーナー", index=False)
            if df_buf is not None:
                df_buf.drop(columns=["スクリーナー"]).to_excel(writer, sheet_name="三角測量スクリーナー", index=False)
            if df_bot is not None:
                df_bot.drop(columns=["スクリーナー"]).to_excel(writer, sheet_name="シグナルハンター", index=False)
    ...
    with dl_col3:
        try:
            ...
            # Try to register a Japanese TrueType font from the Windows font directory
            _jp_font = "Helvetica"
            ...
            sections = [
                ("高配当スクリーナー 合格銘柄", df_div.drop(columns=["スクリーナー"]) if df_div is not None else None),
                ("三角測量スクリーナー 分析結果", df_buf.drop(columns=["スクリーナー"]) if df_buf is not None else None),
                ("シグナルハンター スキャン結果", df_bot.drop(columns=["スクリーナー"]) if df_bot is not None else None),
            ]
