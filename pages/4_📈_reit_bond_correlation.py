...
「過去と比べて今が高利回り水準 = 相対的に割安」というサインです。
    ...
    # ── 総合判定カード ────────────────────────────────────────
    render_verdict_card(kpi["verdict"], kpi["action"])
    ...
    with cat_a:
        ...
        st.metric(
            "分配金利回り（概算）", f"{kpi['dist_yield_cur']:.2f}%",
            delta=f"{kpi['yield_delta']:+.2f}%pt" if not np.isnan(kpi["yield_delta"]) else None,
            help=f"年間分配金（¥{ANNUAL_DIST_YEN:.0f}/口）を現在の市場価格で割った値です。例：価格が2,000円で年間分配金が80円なら利回り4%。数値が高いほど投資の「旨み」が大きい。",
        )
        st.metric(
            f"配当スコア　{kpi['score_eval']}", kpi["score_str"],
            delta=f"1年平均スプレッド比 {kpi['spread_cur'] - kpi['spread_avg']:+.2f}pt",
    ...
    with cat_b:
        ...
        st.metric(
            "イールドスプレッド", f"{kpi['spread_cur']:.2f}%pt",
            delta=f"1年平均比 {kpi['spread_cur'] - kpi['spread_avg']:+.2f}pt" if not np.isnan(kpi["spread_avg"]) else None,
            help="REIT分配金利回りから日本国債10年利回りを引いた差です。スプレッドが広いほど「リスクを取る見返り（プレミアム）」が大きく割安。1.5%pt以上で割安、1.0%pt以下で割高の目安。",
        )
        bond_str = f"{kpi['bond_last']:.3f}%" if not np.isnan(kpi["bond_last"]) else "—"
        chg_str  = f"{kpi['bond_chg']:+.3f}pt" if not np.isnan(kpi["bond_chg"]) else None
    ...
    with cat_c:
        ...
        corr_str_disp  = f"{kpi['corr_val']:.3f} {kpi['corr_icon']}" if not np.isnan(kpi["corr_val"]) else "—"
        corr_delta_str = f"{kpi['corr_delta']:+.3f}" if not np.isnan(kpi["corr_delta"]) else None
        st.metric(
            f"20日相関係数　{kpi['corr_state_short']}", corr_str_disp, delta=corr_delta_str,
            help="-1.0〜+1.0の数値でREIT価格と金利の連動度を示します。マイナス（特に-0.7以下）は「金利上昇→REIT下落」の逆相関が強い状態。ゼロ付近はほぼ無関係、プラスは同じ方向に動く。",
        )
        reit_ret_str = f"{kpi['reit_ret']:+.2%}" if not np.isnan(kpi["reit_ret"]) else "—"
        st.metric(
            "高利回りシグナル", kpi["buy_sig_str"],
            delta=reit_ret_str + " 前日騰落" if not np.isnan(kpi["reit_ret"]) else None,
            delta_color="normal",
...
def render() -> None:
    ...
    kpi = dict(
        ...
        dist_yield_cur=dist_yield_cur, yield_delta=yield_delta,
        spread_cur=spread_cur, spread_avg=spread_avg,
        score_str=score_str, score_eval=score_eval,
        bond_last=bond_last, bond_chg=bond_chg,
        corr_val=corr_val, corr_delta=corr_delta,
        corr_icon=corr_icon, corr_state_short=corr_state_short,
        buy_sig_now=buy_sig_now, buy_sig_str=buy_sig_str,
        reit_ret=reit_ret,
    )
    ...
    with tab_macro:
        _render_macro_tab(df_yield, df_corr, kpi)
